...
//참고 : 자바 프로그래밍 언어에서는 클래스 이름과 파일명이 동일해야 한다.
...
//		isum = a + c;		//결과가 실수이므로 정수형식 변수에 저장을 할 수 없다.
		
//		형변환(캐스팅, casting) : 정수 + 실수 결과를 꼭 정수 변수에 저장해야한다면 실수를 정수로 변환해서 연산을 한다.
//			→ 프로그래머가 정해서 강제로 형변환 → ()안에 변환시킬 형식 작성
...
//		강제 형변환(캐스팅) 예시 : 정수끼리의 나눗셈 결과를 실수로 구하는 처리조건일 때...
//				→ 정수 1개를 실수로 바꿔준다.
