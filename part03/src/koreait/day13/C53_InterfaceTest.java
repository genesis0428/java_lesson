...
//참조 타입에 따른 메소드 실행	: z1, z2, z3이 어떤 메소드 실행가능한지?
		...
		z2.methodA();
		z2.methodB(23);
//		z2.name("쯔위");	//오류 : InterfaceB의 추상메소드로 실행못함
		
//		z3.methodA();	//오류 : InterfaceA의 추상메소드로 실행못함
//		z3.methodB(23);	//오류 : InterfaceA의 추상메소드로 실행못함
		z3.name("쯔위");
...
//다형성 구현 예		
		iAmethod(x2);
//		iAmethod(y2);	//오류 : InterfaceA의 구현체가 아니라서 오류
		iAmethod(z2);
		iAmethod(x1);
		iAmethod(z1);
//		iAmethod(z3);	//오류 : 변수 참조타입이 InterfaceB
		...
		iBmethod(y2);
		iBmethod(z1);
//		iBmethod(z2);	//오류 : 변수 참조타입이 InterfaceA이면 오류
		iBmethod(z3);
